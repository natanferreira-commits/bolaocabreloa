...
const CONFETE = Array.from({ length: 26 }, (_, i) => ({
  ...
  t: 1.6 + (i % 5) * 0.25,
  r: (i * 53) % 360,
  c: i % 3 === 0 ? "#fff" : i % 3 === 1 ? "var(--gold)" : "var(--hl)",
}));
function Confete() {
  return (
...
function Landing({ onStart }) {
  ...
  const labelNaBase = landing.labelPosicao === "base";
  const rotulo = landing.label.split("•").map((l, i) => (
  ...
  return (
    <div className="page">
      ...
      <section
        className={`hero-img${landing.heroImage ? " com-imagem" : ""}${recorte ? " recorte" : ""}${labelNaBase ? " base-copy" : ""}`}
        style={heroStyle}
      >
        {recorte && landing.heroImage && <div className="hero-foto" style={fotoStyle} aria-hidden="true" />}
        <Particulas />
        {!labelNaBase && <div className="hero-top">{rotulo}</div>}
        <div className="wrap hero-copy">
          {labelNaBase && <div className="hero-rotulo">{rotulo}</div>}
          <h1>
